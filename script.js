...
function mostrar() {
    ...
    if(tipo === 'lista') {
        ...
        html += dados.map((d, i) => {
            ...
            let nomeVeiculo = d.NOME_VEICULO || ''; // AQUI PEGA O NOME
    ...
    if(tipo === 'consumo') {
        let resumo = {};
        dados.forEach(d => {
            let nomeVeiculo = d.NOME_VEICULO || '';
            if(!resumo[d.PLACA]) resumo[d.PLACA] = {nome: nomeVeiculo, litros: 0, valor: 0, kmInicial: 999999, kmFinal: 0};
