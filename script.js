...
window.cadastrarVeiculo = cadastrarVeiculo; // FORÇA FICAR GLOBAL PRO ONCLICK
window.abrirModal = abrirModal;
window.fecharModal = fecharModal;
window.abrirModalVeiculos = abrirModalVeiculos;
window.fecharModalVeiculos = fecharModalVeiculos;
window.editarVeiculo = editarVeiculo;
window.excluirVeiculo = excluirVeiculo;
window.mostrar = mostrar;
window.gerarPDF = gerarPDF;
window.mudaTipoRel = mudaTipoRel;
...
function atualizarSelects() {
    ...
    if(filtroVeiculo) {
        ...
        filtroVeiculo.innerHTML = filtro;
    }
    mostrar();
}

function cadastrarVeiculo() {
    const placaInput = document.getElementById('placaNova');
    const nomeInput = document.getElementById('nomeVeiculo');
    let placa = placaInput.value.toUpperCase().trim();
    let nome = nomeInput.value.toUpperCase().trim();
    if(!placa ||!nome) return alert('PREENCHA PLACA E NOME');
    if(veiculos.find(v => v.placa === placa)) return alert('PLACA JÁ CADASTRADA: ' + placa);
...
function abrirModalVeiculos() { document.getElementById('modalVeiculos').style.display = 'flex'; listarVeiculosCadastrados(); }
function fecharModalVeiculos() { document.getElementById('modalVeiculos').style.display = 'none'; }

function listarVeiculosCadastrados() {
    let html = '';
    if(veiculos.length === 0) { html = '<p>NENHUM VEÍCULO CADASTRADO</p>'; } 
    else {
        veiculos.forEach((v, index) => {
            html += `<div class="item-veiculo"><span><b>${v.nome}</b> - ${v.placa}</span><div class="botoes-lista"><button class="btn-editar" onclick="editarVeiculo(${index})">EDITAR</button><button class="btn-excluir" onclick="excluirVeiculo(${index})">EXCLUIR</button></div></div>`;
        });
    }
...
function editarVeiculo(index) {
    ...
    if(novoNome) {
        ...
        if(novaPlaca) {
            abastecimentos.forEach(a => { if(a.PLACA === v.placa) { a.PLACA = novaPlaca.toUpperCase(); a.NOME_VEICULO = novoNome.toUpperCase(); }});
...
async function carregarDados() {
    try {
        let res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({action: "getAll"}), headers: {'Content-Type': 'text/plain'} });
        abastecimentos = await res.json();
        abastecimentos = abastecimentos.filter(a => a.DATA);
    } catch(e) { abastecimentos = []; }
    mostrar();
}
...
async function salvar() {
    ...
    let novo = { action: "add", placa: veiculoSelecionado.placa, data: document.getElementById('data').value, nome: veiculoSelecionado.nome, motorista: document.getElementById('motorista').value.toUpperCase().trim(), litros: parseFloat(document.getElementById('litros').value), valor: parseFloat(document.getElementById('valor').value), km: parseInt(document.getElementById('km').value) };
    ...
    document.getElementById('motorista').value = ''; document.getElementById('litros').value = ''; document.getElementById('valor').value = ''; document.getElementById('km').value = '';
    fecharModal();
    carregarDados();
}
...
function mostrar() {
    ...
    if(tipo === 'lista') {
        ...
        html += dados.map((d, i) => { let indexOriginal = abastecimentos.indexOf(d); return `<tr><td>${d.DATA.split('-').reverse().join('/')}</td><td>${d.NOME_VEICULO}</td><td>${d.PLACA}</td><td>${d.MOTORISTA}</td><td>${d.LITROS}</td><td>R$ ${parseFloat(d.VALOR).toFixed(2)}</td><td>${d.KM}</td><td><button onclick="pedirSenha(${indexOriginal})" class="btn-excluir">EXCLUIR</button></td></tr>` }).join('');
        html += '</table>';
    }
    if(tipo === 'consumo') {
        let resumo = {};
...
function pedirSenha(index) { let senha = prompt('DIGITE A SENHA PARA EXCLUIR ABASTECIMENTO:'); if(senha === SENHA) excluirItem(index); else if(senha) alert('SENHA INCORRETA!'); }
async function excluirItem(index) { if(!confirm('TEM CERTEZA QUE DESEJA EXCLUIR?')) return; await fetch(API_URL, { method: 'POST', body: JSON.stringify({action: "delete", row: index}), headers: {'Content-Type': 'text/plain'} }); alert('EXCLUÍDO!'); carregarDados(); }
function gerarPDF() { let conteudo = document.getElementById('resultado').innerHTML; if(conteudo.trim() === '') return alert('GERAR RELATÓRIO PRIMEIRO'); let dataAtual = new Date().toLocaleDateString('pt-br'); let htmlImpressao = `<html><head><title>Relatorio Frota - ${dataAtual}</title><style>body{font-family: 'Segoe UI', Arial; padding:20px} h2{text-align:center; color:#2563eb} table{width:100%; border-collapse:collapse; margin-top:15px} th,td{border:1px solid #cbd5e1; padding:8px; text-align:left; font-size:12px} th{background:#2563eb; color:#fff}</style></head><body><h2>RELATORIO DE ABASTECIMENTO - ${dataAtual}</h2>${conteudo}<script>window.onload = function(){ window.print(); }</script></body></html>`; let janela = window.open('', '_blank'); janela.document.write(htmlImpressao); janela.document.close(); }
function mudaTipoRel() { let tipo = document.getElementById('tipoRel').value; document.getElementById('filtroVeiculo').style.display = tipo === 'consumo'? 'none' : 'block'; }